...
const renderValue = (value: ProcessedValue, level = 0, compact = false): ReactElement => {
      ...
      <div className="flex flex-col gap-1.5 mt-1.5">
        {value.items?.map((item, idx) => (
          <div key={`array-item-${idx}`} className="flex items-center gap-2">
            ...
            <div className="flex-1 min-w-0">{renderValue(item, level, compact)}</div>
          </div>
        ))}
      </div>
    )
  ...
  return (
    <span className="text-sm font-mono truncate">
      {value.type === "string" ? `"${String(value.value)}"` : String(value.value)}
    </span>
  )
}
...
export function ObjectCard({ value, level = 0, compact = false }: ObjectCardProps): ReactElement {
  ...
  return (
    <Card className={`w-full border shadow-sm ${level > 0 ? "mt-2" : ""}`}>
      <CardContent className={`p-3 space-y-2 ${compact ? "overflow-y-auto" : ""}`}>
        {Object.entries(obj).map(([key, val], idx) => (
          <div key={`object-prop-${key}-${idx}`} className="flex flex-col">
            <div className="flex items-center gap-4 min-w-0">
              ...
              <div className="flex-1 min-w-0 overflow-hidden">{renderValue(val, level, compact)}</div>
            </div>
          </div>
        ))}
      </CardContent>
