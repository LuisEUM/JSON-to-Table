...
import { groupColumns, type ProcessedItem, type GroupedColumns, type ProcessedRow } from "../data-processor"

const createColumnDef = (item: ProcessedItem): ColumnDef<ProcessedRow> => {
  const isSortable = ["string", "número entero", "número decimal", "boolean", "fecha"].includes(item.type)
      ...
      return (
        <div className="flex flex-col items-center gap-1">
          ...
          <div className="flex items-center gap-1">
            {isSortable && (
              <div className="flex">
                <Button
                  variant="ghost"
                  size="sm"
                  className={`h-6 w-6 p-0 ${isSorted === "asc" ? "text-primary" : "text-muted-foreground"}`}
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  className={`h-6 w-6 p-0 ${isSorted === "desc" ? "text-primary" : "text-muted-foreground"}`}
            ...
            <Button variant="ghost" size="sm" className="h-6 w-6 p-0" onClick={() => column.toggleVisibility()}>
